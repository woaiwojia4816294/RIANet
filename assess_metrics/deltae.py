...
def rgb2lab_matrix(colors):
    ...
    RGBs = colors / 255.0  # 每个元素置于 0-1 之间
    Xs = RGBs[:, 0] * 0.4124 + RGBs[:, 1] * 0.3576 + RGBs[:, 2] * 0.1805  # ndarray 是 1780000
    Ys = RGBs[:, 0] * 0.2126 + RGBs[:, 1] * 0.7152 + RGBs[:, 2] * 0.0722
    Zs = RGBs[:, 0] * 0.0193 + RGBs[:, 1] * 0.1192 + RGBs[:, 2] * 0.9505
    XYZs = np.vstack((Xs, Ys, Zs)).transpose()  # np.vstack 是 1780000,3 再 transpose 一下

    XYZs[:, 0] = XYZs[:, 0] / (95.045 / 100.0)
    XYZs[:, 1] = XYZs[:, 1] / (100.0 / 100.0)
    XYZs[:, 2] = XYZs[:, 2] / (108.875 / 100.0)
    L = np.zeros((n, 3), dtype='float')  # 亮度分量是 1780000,3
    for i in range(0, 3):
        v = XYZs[:, i]
...
def delta_e_1976(Lab1, Lab2):
    ...
    dela = Lab1['a'] - Lab2['a']
    delb = Lab1['b'] - Lab2['b']
    result = math.sqrt((np.mean(delL * delL + dela * dela + delb * delb)))
    return result
...
def delta_e_2000(Lab1, Lab2, verbose=False, test=False, formula='Rochester'):
    ...
    t = 1.0 - 0.17 * math.cos((math.pi * (hBarPrime - 30.0)) / 180.0) + 0.24 * math.cos(
        (math.pi * (2.0 * hBarPrime)) / 180.0) + \
    ...
    DE2000 = math.sqrt((dLPrime / (kL * sL)) * (dLPrime / (kL * sL)) + (dCPrime / (kC * sC)) * (dCPrime / (kC * sC)) +
                       (dHPrime / (kH * sH)) * (dHPrime / (kH * sH)) + (dCPrime / (kC * sC)) * (
                               dHPrime / (kH * sH)) * rT)

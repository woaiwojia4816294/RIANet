...
class AttentionModule(nn.Module):
    def __init__(self):
        ...
        # self.down3 = nn.PixelUnshuffle(4)
    ...
    def forward(self, x):  # x.shape 2,32,320,320
        pool_feature1 = self.pool1(x)  # 2,32,240,480
        pool_feature2 = self.pool2(x)  # 2,32,60,120
        pool_feature3 = self.pool3(x)  # 2,32,26,53
        # down_feature1 = x  # 2,32,320,320
        # down_feature2 = self.down2(x)  # 2,128,160,160
        # down_feature3 = self.down3(x)  # 2,512,80,80
        ...
        feature_combine = torch.cat((pool_feature1_up, pool_feature2_up, pool_feature3_up), 1)  # 2,96,320,320
        attention = self.model_attention(feature_combine)  # 2,10,320,320
        image = self.model_image(feature_combine)  # 2,30,320,320
        ...
        for i in range(len(attention[:, 0, 0, 0])):
            ...
            output10 = image10 * attention10  # 1,3,320,320

            output = output1 + output2 + output3 + output4 + output5 + \
                     output6 + output7 + output8 + output9 + output10  # 1,3,320,320
            img.append(output)  # img 里面是 bs 个 1,3,320,320

        img = torch.cat(img, dim=0)
        return img  # 2,3,320,320


class AttenNet(nn.Module):
    def __init__(self, fem_channel=32, block_num=9):
    ...
    def forward(self, x):  # 2,3,320,320
        x = self.FEM(x)  # 2,32,320,320
        x = self.activation(x)
        atten_features = []
        atten_features.append(self.attention(x))  # 2,3,320,320

        for i in range(self.block_num):
            x = self.blocks[2 * i](x)  # 2,32,320,320
            atten_features.append(self.blocks[2 * i + 1](x))
            # 上面的 atten_features 是 10 个 2,3,320,320

        atten_feature_batch = torch.cat(atten_features, dim=1)
...
class ChannelAtten(nn.Module):
    def __init__(self, in_channel=32, out_channel=32):
        ...
        self.fc = nn.Sequential(nn.Linear(in_channel, out_channel),
                                nn.ReLU(),
    ...
    def forward(self, x):
        bs = x.shape[0]
        c, h, w = x.shape[1:4]  # 12 512 512
        avg = nn.AvgPool2d([h, w], stride=None)  #
        y = avg(x)  # y 2,12,1,1
        y = y.view(bs, 1, 1, -1)  # 1,1,1,12
        y = self.fc(y)  # 1,1,1,12  self.fc 是两层全连接
        y = y.view(bs, -1, 1, 1)  # 1,12,1,1
        output = x * y  # 1,12,512,512  1,12,1,1
        return output  # 1,12,512,512

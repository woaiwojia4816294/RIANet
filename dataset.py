...
class MBLLENData(data.Dataset):
    def __init__(self, data_dir, mode1, mode2, transform):
        ...
        self.lows = glob(data_dir + '/%s_%s/*.*' % (mode1, mode2))  # train_lowlight  test_lowlight
        self.highs = glob(data_dir + '/%s/*.*' % mode1)  # train  test
        assert len(self.lows) == len(self.highs)  # 至少数量是一一对应的？
